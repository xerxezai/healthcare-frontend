            ...
            <Col md={6} className="position-relative d-flex align-items-center">
              <div className="sign-in-from d-flex flex-column justify-content-start py-3">
                ...
                <Form className="mt-2" onSubmit={handleSubmit}>
                  <div className="mb-3">
                    <ProgressBar now={((step + 1) / steps.length) * 100} label={steps[step]} />
                  </div>
                  {step === 0 && (<><Form.Group className="form-group">
                    <Form.Label>Full Name</Form.Label>
                    <Form.Control type="text" name="full_name" value={formData.full_name} onChange={handleChange} isInvalid={!!formErrors.full_name} className="mb-0" placeholder="Enter your full name" />
                    {formErrors.full_name && (<div className="text-danger mt-1 small">{formErrors.full_name.join(', ')}</div>)}
                  </Form.Group>
                  <Form.Group className="form-group">
                    <Form.Label>Email</Form.Label>
                    <Form.Control type="email" name="email" value={formData.email} onChange={handleChange} isInvalid={!!formErrors.email} className="mb-0" placeholder="Enter your email" />
                    {formErrors.email && (<div className="text-danger mt-1 small">{formErrors.email.join(', ')}</div>)}
                  </Form.Group>
                  <Form.Group className="form-group">
                    <Form.Label>Password</Form.Label>
                    <Form.Control type="password" name="password" value={formData.password} onChange={handleChange} isInvalid={!!formErrors.password} className="mb-0" placeholder="Enter your password" />
                    {formErrors.password && (<div className="text-danger mt-1 small">{formErrors.password.join(', ')}</div>)}
                  </Form.Group></>)}
                  {step === 1 && (<><Form.Group className="form-group">
                    <Form.Label>Role <span title="Select your role in the healthcare system.">*</span></Form.Label>
                    <Form.Select name="role" value={formData.role} onChange={handleChange} isInvalid={!!formErrors.role} className="mb-0">
                    ...
                    <Form.Text className="text-muted">Your role determines your dashboard and available features.</Form.Text>
                    {formErrors.role && (<div className="text-danger mt-1 small">{formErrors.role.join(', ')}</div>)}
                  </Form.Group>
                  {showRoleSpecificFields && (<Form.Group className="form-group">
                    <Form.Label>Specialization</Form.Label>
                    <Form.Control type="text" name="specialization" value={formData.specialization} onChange={handleChange} className="mb-0" placeholder="Enter your specialization (optional)" />
                    <Form.Text className="text-muted">E.g., Cardiology, Radiology, Dentistry, etc.</Form.Text>
                  </Form.Group>)}</>)}
                  {step === 2 && (<><Form.Group className="form-group">
                    <Form.Label>Phone Number</Form.Label>
                    <Form.Control type="tel" name="phoneNumber" value={formData.phoneNumber} onChange={handleChange} className="mb-0" placeholder="Enter your phone number" />
                    <Form.Text className="text-muted">For account recovery and notifications.</Form.Text>
                  </Form.Group></>)}
                  {step === 3 && (<><Alert variant="info">
                    <strong>Review your details before submitting.</strong>
                  </Alert>
                  {/* Display summary here if desired */}</>)}
                  <div className="d-flex justify-content-between mt-3">
                    {step > 0 && <Button variant="secondary" onClick={handlePrev}>Back</Button>}
                    {step < steps.length - 1 && <Button variant="primary" onClick={handleNext}>Next</Button>}
                    {step === steps.length - 1 && <Button type="submit" className="btn btn-primary" disabled={isLoading}>{isLoading ? 'Creating Account...' : 'Sign Up'}</Button>}
                  </div>
                  <Alert variant="info" className="mt-3">
                    <strong>Tip:</strong> After registration, check your email for a verification link. You’ll be guided through onboarding to set up your profile and explore platform features.
                  </Alert>
                  ...
                  <div className="mt-3">
                    ...
                    <button type="button" className="btn btn-outline-danger w-100 mb-2">Sign up with Google</button>
                    <button type="button" className="btn btn-outline-primary w-100">Sign up with Facebook</button>
                  </div>
                </Form>
              </div>
            </Col>
