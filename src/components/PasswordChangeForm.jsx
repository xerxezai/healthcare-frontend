...
import { Eye, EyeOff, Lock, Key, AlertCircle, CheckCircle, Shield } from 'lucide-react';

const PasswordChangeForm = ({ userEmail, onSuccess, onCancel, showCancelButton = true }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await apiClient.post(buildApiUrl(AUTH_ENDPOINTS.CHANGE_PASSWORD), formData);
    ...
    return (
        <div className="max-w-md w-full bg-white rounded-xl shadow-xl p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* Buttons */}
                <div className={`flex ${showCancelButton ? 'space-x-4' : ''} pt-6`}>
                    {showCancelButton && (
                        <button
                            type="button"
                            onClick={onCancel}
                    ...
                    <button
                        ...
                        className={`${showCancelButton ? 'flex-1' : 'w-full'} py-3 px-4 bg-blue-600 text-white rounded-lg font-medium hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center transition-colors`}
                    >
                        {loading ? (
